...
const allowedOrigins = [
  'https://mi-agenda-app-db.web.app',
  'https://agenda-app-5sh6.onrender.com',
  'http://localhost:3000',
  'http://localhost:5500'
];
...
// Middleware para headers CORS manuales
...
// =========================
// RUTAS PARA CATEGORÍAS (ENTIDAD 2) - CRUD COMPLETO
// =========================
...
// PUT actualizar categoría por ID - ✅ IMPLEMENTADO
app.put('/api/categorias/:id', validateToken, (req, res) => {
  try {
    ...
    if (nombre !== undefined) {
      data.categorias[categoriaIndex].nombre = sanitizeInput(nombre);
    }
    if (color !== undefined) {
      data.categorias[categoriaIndex].color = color;
    }
...
// DELETE categoría por ID - ✅ IMPLEMENTADO
  ...
  try {
    ...
    if (tareasConCategoria.length > 0) {
      return res.status(400).json({ 
        ...
        tareasAsociadas: tareasConCategoria.length,
        sugerencia: 'Primero cambia la categoría de las tareas o elimínalas'
      });
    }
...
app.listen(PORT, () => {
  ...
  console.log('🏷️ ENTIDADES COMPLETAS:');
  console.log('   • Tareas: GET, POST, PUT, DELETE');
  console.log('   • Categorías: GET, POST, PUT, DELETE ✅');
  console.log('🔑 Credenciales: admin / admin123');
  console.log('🔑 Token: demo-token-123');
  console.log('=========================================');
});
